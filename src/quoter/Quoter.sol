...
contract Quoter is IQuoter, IUnlockCallback {
    ...
    function quoteExactInput(
        QuoteExactParams calldata params
    )
        external
        returns (
            int128[] memory deltaAmounts,
        ...
        )
    {
        try
            manager.unlock(
                abi.encodeWithSelector(this._quoteExactInput.selector, params)
            )
        {} catch (bytes memory reason) {
        ...
        )
    {
        try
            manager.unlock(
                abi.encodeWithSelector(
                    this._quoteExactOutputSingle.selector,
                    params
                )
            )
        {} catch (bytes memory reason) {
        ...
        )
    {
        try
            manager.unlock(
                abi.encodeWithSelector(this._quoteExactOutput.selector, params)
            )
        {} catch (bytes memory reason) {
            return _handleRevert(reason);
        }
        ...
        )
    {
        ...
        (deltaAmounts, sqrtPriceX96After, initializedTicksLoaded) = abi.decode(
            reason,
    ...
    {
        reason = validateRevertReason(reason);
        (deltaAmounts, sqrtPriceX96AfterList, initializedTicksLoadedList) = abi
            .decode(reason, (int128[], uint160[], uint32[]));
    }

    /// @dev quote an ExactInput swap along a path of tokens, then revert with the result
    function _quoteExactInput(
        QuoteExactParams calldata params
    ) public selfOnly returns (bytes memory) {
        ...
        for (uint256 i = 0; i < pathLength; i++) {
            (PoolKey memory poolKey, bool zeroForOne) = params
                .path[i]
                .getPoolAndSwapDirection(
                    i == 0 ? params.exactCurrency : cache.prevCurrency
                );
            ...
            (cache.curDeltas, cache.sqrtPriceX96After, cache.tickAfter) = _swap(
                poolKey,
                zeroForOne,
                -int256(i == 0 ? params.exactAmount : cache.prevAmount),
                0,
            ...
            result.initializedTicksLoadedList[i] = PoolTicksCounter
        ...
        bytes memory r = abi.encode(
            result.deltaAmounts,
            result.sqrtPriceX96AfterList,
            result.initializedTicksLoadedList
        );
        assembly {
    ...
    function _quoteExactInputSingle(
        ...
        uint32 initializedTicksLoaded = PoolTicksCounter
            .countInitializedTicksLoaded(
                manager,
                params.poolKey,
                tickBefore,
    ...
    function _quoteExactOutput(
        ...
        for (uint256 i = pathLength; i > 0; i--) {
            curAmountOut = i == pathLength
                ? params.exactAmount
                : cache.prevAmount;
            amountOutCached = curAmountOut;

            (PoolKey memory poolKey, bool oneForZero) = PathKeyLibrary
                .getPoolAndSwapDirection(
                    params.path[i - 1],
        ...
        )
    {
        ...
        deltas = manager.swap(
            ...
            IPoolManager.SwapParams({
                zeroForOne: zeroForOne,
                amountSpecified: amountSpecified,
                sqrtPriceLimitX96: _sqrtPriceLimitOrDefault(
                    sqrtPriceLimitX96,
                    zeroForOne
                )
            }),
        ...
        if (
            amountOutCached != 0 &&
            amountOutCached !=
            uint128(zeroForOne ? deltas.amount1() : deltas.amount0())
        ) {
    ...
    function _sqrtPriceLimitOrDefault(
        uint160 sqrtPriceLimitX96,
        bool zeroForOne
