...
contract VolatilityFeesHook is BaseHook {
    ...
    // {
    //     // StateView state = new StateView(manager);
    //     (uint160 sqrtPriceX961, int24 tick1, uint24 protocolFee1, uint24 lpFee1) = poolManager.getSlot0(key.toId());
    ...
    //     uint256 tickBitmap = poolManager.getTickBitmap(key.toId(), wordPos);

    //     // Pool.State memory self;
    //     uint128 self_liquidity = poolManager.getLiquidity(key.toId());
    ...
    //         uint256 tickBitmap2 = poolManager.getTickBitmap(key.toId(), wordPos2);
    ...
    //             if (step.initialized) {
    //                 (uint128 liquidityGross, int128 liquidityNet) = poolManager.getTickLiquidity(key.toId(), step.tickNext);
